...
def extract_structured_fields(role: str, mode: str, model: str) -> str:
    prompt = (
...
selected_model = st.selectbox("Choose the GPT model for analysis", ["gpt-4o", "gpt-3.5-turbo"])
...
if st.button("Run Matching") and job_description and uploaded_files:
    with st.spinner("Processing..."):
        ...
        jd_keywords = extract_structured_fields(job_description, "keywords", selected_model)
        jd_experience = extract_structured_fields(job_description, "experience", selected_model)
        jd_soft = extract_structured_fields(job_description, "soft skills", selected_model)
        ...
        for file, name, text in zip(uploaded_files, resume_names, resume_texts):
            ...
            r_keywords = extract_structured_fields(text, "keywords", selected_model)
            r_experience = extract_structured_fields(text, "experience", selected_model)
            r_soft = extract_structured_fields(text, "soft skills", selected_model)
            ...
            st.session_state.results.append((name, final_score, kw_score, exp_score, soft_score, email, url))

        st.session_state.results.sort(key=lambda x: x[1], reverse=True)
        st.success("Matching complete!")

st.subheader("📋 Top Matches")
for tup in st.session_state.results:
